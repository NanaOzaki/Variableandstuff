...
//£££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££//MP
  //Checks whether surface tension will be used
  SurfCoef=Surft;
//£££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££//MP
...
//£££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££//MP
  ...
  if(TPhase==FLOW_Multi){
	UpdateRhopMP(Np,Npb,Posc,Velrhopc,Codec);
	//-Copies multiphase data.
	PhaseDatac=JSph::GetPhaseData();
  }
//£££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££//MP
...
//==============================================================================
/// Interaccion para el calculo de fuerzas.
/// Interaction to calculate forces. -££//MP
//==============================================================================
void JSphCpuSingle::Interaction_ForcesMP(TpInter tinter){
  const char met[]="Interaction_ForcesMP";
  PreInteraction_ForcesMP(tinter);
  TmcStart(Timers,TMC_CfForces);

  //-Interaction of Fluid-Fluid/Bound & Bound-Fluid (forces and DEM) / Interaccion Fluid-Fluid/Bound & Bound-Fluid (forces and DEM).
  float viscdt=0;
  if(Psimple)JSphCpu::InteractionSimple_ForcesMP(Np,Npb,NpbOk,CellDivSingle->GetNcells(),CellDivSingle->GetBeginCell(),CellDivSingle->GetCellDomainMin(),Dcellc,PsPosc,Velrhopc,Idpc,Codec,Pressc,viscdt,Arc,Acec,Deltac,SpsTauc,SpsGradvelc,Surfc,PhaseDatac,ShiftPosc,ShiftDetectc);
  else JSphCpu::Interaction_ForcesMP(Np,Npb,NpbOk,CellDivSingle->GetNcells(),CellDivSingle->GetBeginCell(),CellDivSingle->GetCellDomainMin(),Dcellc,Posc,Velrhopc,Idpc,Codec,Pressc,viscdt,Arc,Acec,Deltac,SpsTauc,SpsGradvelc,Surfc,PhaseDatac,ShiftPosc,ShiftDetectc);
  ...
  //-Calculates maximum value of Ace.
  AceMax=ComputeAceMaxOmp(PeriActive!=0,Np-Npb,Acec+Npb,Codec+Npb);
  //-Calculates maximum value of Rhop.
  RhopMax=ComputeRhopMaxOmpMP(PeriActive!=0,Np-Npb,Velrhopc+Npb,Codec+Npb);
...
//£££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££//MP
//==============================================================================
/// Calculates shifting according to Mokos et al. Only used for multiphase flows. -££//MP
//==============================================================================
void JSphCpuSingle::RunShifting_MP(double dt){
	TmcStart(Timers,TMC_SuShifting);
	if(Psimple)JSphCpu::RunShiftingSimpleMP(Np,Npb,NpbOk,CellDivSingle->GetNcells(),CellDivSingle->GetBeginCell(),CellDivSingle->GetCellDomainMin(),Dcellc,PsPosc,Velrhopc,Idpc,Codec,Acec,Surfc,PhaseDatac,TShifting,ShiftPosc,ShiftDetectc,dt);
	else JSphCpu::RunShiftingMP(Np,Npb,NpbOk,CellDivSingle->GetNcells(),CellDivSingle->GetBeginCell(),CellDivSingle->GetCellDomainMin(),Dcellc,Posc,Velrhopc,Idpc,Codec,Acec,Surfc,PhaseDatac,TShifting,ShiftPosc,ShiftDetectc,dt);
...
//£££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££//MP
//==============================================================================
/// Devuelve el valor maximo de rhop (modulo).
/// Returns maximum value of rhop (modulus). -££//MP
//==============================================================================
double JSphCpuSingle::ComputeRhopMaxSeqMP(const bool checkcodenormal,unsigned np,const tfloat4* velrhop,const word* code)const{
  float acemax=0;
  const int n=int(np);
...
//£££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££//MP
//==============================================================================
/// Devuelve el valor maximo de rhop (modulo) using OpenMP.
/// Returns maximum value of rhop (modulus) using OpenMP. -££//MP
//==============================================================================
double JSphCpuSingle::ComputeRhopMaxOmpMP(const bool checkcodenormal,unsigned np,const tfloat4* velrhop,const word* code)const{
  const char met[]="ComputeRhopMaxOmpMP";
  double acemax=0;
  #ifdef _WITHOMP
    ...
    else if(np)acemax=ComputeRhopMaxSeqMP(checkcodenormal,np,velrhop,code);
  #else
    if(np)acemax=ComputeRhopMaxSeqMP(checkcodenormal,np,velrhop,code);
  #endif
  return(acemax);
}
//£££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££//MP
...
//-Interaccion //-Interaction
  if(TPhase==FLOW_Multi)Interaction_ForcesMP(INTER_Forces);     //-Multi-phase
  else Interaction_Forces(INTER_Forces);						//-Single-phase
  const double dt=DtVariable(true);				//-Calculate new dt / Calcula nuevo dt
  DemDtForce=dt;								//(DEM)
//-Particle Shifting - Surface Tension
  if(TShifting || SurfCoef){
	   if(TPhase==FLOW_Multi)RunShifting_MP(dt);		//-Multi-phase
	   else RunShifting(dt);							//-Single-phase
  }
//-Update particles using Verlet / Actualiza particulas usando Verlet
  if(TPhase==FLOW_Multi) ComputeVerletMP(dt);			//-Multi-phase
  else ComputeVerlet(dt);								//-Single-phase
//£££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££//MP
...
//-Calculate dt of predictor step / Calcula dt del predictor
  const double ddt_p=DtVariable(false);
//-Particle Shifting - Surface Tension
  if(TShifting){
	   //if(TPhase==FLOW_Multi)RunShifting_MP(dt*.5);			//-Removed shifting for the predictor step for multi-phase flows
	   //else RunShifting(dt*.5);
	  if(TPhase==FLOW_Single)RunShifting(dt*.5);				//-Single-phase
  }
//-Apply Symplectic-Predictor to particles / Aplica Symplectic-Predictor a las particulas
  if(TPhase==FLOW_Multi)ComputeSymplecticPreMP(dt);				//-Multi-phase
  else ComputeSymplecticPre(dt);								//-Single-phase
//£££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££//MP
...
//£££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££//MP
//-Interaccion //-Interaction
  if(TPhase==FLOW_Multi)Interaction_ForcesMP(INTER_ForcesCorr);     //-Multi-phase
  else Interaction_Forces(INTER_ForcesCorr);						//-Single-phase
//-Calculate dt of corrector step / Calcula dt del corrector
...
//-Apply Symplectic-Corrector to particles / Aplica Symplectic-Corrector a las particulas
  if(TPhase==FLOW_Multi)ComputeSymplecticCorrMP(dt);				//-Multi-phase
  else ComputeSymplecticCorr(dt);									//-Single-phase
//£££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££//MP
  if(CaseNfloat)RunFloating(dt,false);    //-Control of floating bodies / Gestion de floating bodies
  PosInteraction_Forces();                //-Free memory used for interaction / Libera memoria de interaccion
...
void JSphCpuSingle::Run(std::string appname,JCfgRun *cfg,JLog2 *log){
  ...
  if(TPhase==FLOW_Single) SaveData();
  else SaveDataMP();
...
//==============================================================================
/// Genera los ficheros de salida de datos.
/// Generates files with output data for the multi-phase flows. -££//MP
//==============================================================================
void JSphCpuSingle::SaveDataMP(){
  const bool save=(SvData!=SDAT_None&&SvData!=SDAT_Info);
  const unsigned npsave=Np-NpbPer-NpfPer; //-Keep the periodic particles if they exist / Resta las periodicas si las hubiera.
  TmcStart(Timers,TMC_SuSavePart);
  //-Collect particle values in original order / Recupera datos de particulas en orden original.
  ...
  if(save){
    ...
    vel=ArraysCpu->ReserveFloat3();
    rhop=ArraysCpu->ReserveFloat();
	code=ArraysCpu->ReserveWord();
    unsigned npnormal=GetParticlesData(Np,0,true,PeriActive!=0,idp,pos,vel,rhop,code);
    if(npnormal!=npsave)RunException("SaveData","The number of particles is invalid.");
  }
  //-Gather additional information / Reune informacion adicional.
  StInfoPartPlus infoplus;
  ...
  //-Record particle values / Graba datos de particulas.
  const tdouble3 vdom[2]={OrderDecode(CellDivSingle->GetDomainLimits(true)),OrderDecode(CellDivSingle->GetDomainLimits(false))};
  JSph::SaveDataMP(npsave,idp,pos,vel,rhop,code,1,vdom,&infoplus);
  //-Libera memoria para datos de particulas.
  ArraysCpu->Free(idp);
  ArraysCpu->Free(pos);
  ArraysCpu->Free(vel);
