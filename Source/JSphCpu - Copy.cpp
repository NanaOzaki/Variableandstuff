...
void JSphCpu::PreInteractionVars_ForcesMP(TpInter tinter,unsigned np,unsigned npb,word *code){
  ...
  //-Prepare values of rhop for interaction / Prepara datos derivados de rhop para interaccion.
  float rhop0,cs0,gamma; unsigned phasetype; float rhop0max=0.f;
  const int n=int(np); 
...
void JSphCpu::PreInteraction_ForcesMP(TpInter tinter){
  ...
  if(TVisco==VISCO_LaminarSPS)SpsGradvelc=ArraysCpu->ReserveSymatrix3f();
//£££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££//MP
  if(Surft) Surfc=ArraysCpu->ReserveSymatrix3f();
//£££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££//MP
...
//£££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££//MP
//==============================================================================
///Restrict shifting on the free surface
//==============================================================================
void JSphCpu::ShiftingFreeSurface(const tfloat3 concgr,tfloat3 &congr)const{
	...
	if(ShiftFSGrad.x || ShiftFSGrad.y || ShiftFSGrad.z){
		congr.x+=0.1f*(dcdn*norm.x-ShiftFSGrad.x)*norm.x;
		congr.y+=0.1f*(dcdn*norm.y-ShiftFSGrad.y)*norm.y;
		congr.z+=0.1f*(dcdn*norm.z-ShiftFSGrad.z)*norm.z;
	}
}
//£££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££//MP
...
/// Perform interaction between particles. Bound-Fluid/Float
//==============================================================================
template<bool psimple,TpKernel tker,TpFtMode ftmode> void JSphCpu::InteractionForcesBoundMP
  (unsigned n,unsigned pinit,tint4 nc,int hdiv,unsigned cellinitial
  ,const unsigned *beginendcell,tint3 cellzero,const unsigned *dcell
  ,const tdouble3 *pos,const tfloat3 *pspos,const tfloat4 *velrhop,const word *code,const unsigned *idp
  ,float &viscdt,float *ar,StPhaseData *phasedata)const
{
  //-Initialize viscth to calculate max viscdt with OpenMP / Inicializa viscth para calcular visdt maximo con OpenMP.
  float viscth[MAXTHREADS_OMP*STRIDE_OMP];
  for(int th=0;th<OmpThreads;th++)viscth[th*STRIDE_OMP]=0;
        ...
        for(unsigned p2=pini;p2<pfin;p2++){
          ...
          if(rr2<=Fourh2 && rr2>=ALMOSTZERO){
            ...
            if(compute){
              ...
              const float p_vpm=-prs*massp2*ftmassp1;
              acep1.x+=p_vpm*frx; acep1.y+=p_vpm*fry; acep1.z+=p_vpm*frz;
			  //if(phasetype1==1 && phasetype2==1){ //C&L fluid selection v1
			  if(phasetype1==1){ //C&L fluid selection v2
				//const float rhopmp=rhopp1*rhopp1+velrhop2.w*velrhop2.w; //C&L equation v1
				const float rhopmp=2.f*rhop01*rhop01;// C&L equation v2
				const float airco=1.5f*MPCoef*H*9.81f*rhop0max/rhop01;
				const float cohterm=airco*rhopmp*massp2*ftmassp1/(rhopp1*velrhop[p2].w);
				acep1.x+=cohterm*frx; //extra term for gases
				acep1.y+=cohterm*fry; 
				acep1.z+=cohterm*frz;
			  }
            }
            ...
            //  shiftdetectp1-=massrhop*(drx*frx+dry*fry+drz*frz);
            //}
			//-Shifting Correction Mokos et al.
			const float massrhop=massp2/velrhop[p2].w;
			//const bool noshift=(boundp2 && (tshifting==SHIFT_NoBound || (tshifting==SHIFT_NoFixed && CODE_GetType(code[p2])==CODE_TYPE_FIXED)));
			//if(shift) shiftdetectp1=(noshift? shiftdetectp1+FLT_MAX:massrhop*wab);
			if(shift)shiftdetectp1+=massrhop*wab;

			//===== Colour function gradient ===== //used to compute surface tension
			if (surf1){//(tsurf==SURFACE_Include){
			//float col=0.f;
			//float volsum=(vol*vol+vol1*vol1)/vol;
				//if (surftype1!=surftype2){
					//col=velrhop2.w/(rhopp1+velrhop2.w); //alternative formulation for the colour function from Hu & Adams 2010
					//gradcol.x+=volsum*col*frx;
					//gradcol.y+=volsum*col*fry;
					//gradcol.z+=volsum*col*frz;
  ...
  ,TpShifting tshifting,tfloat3 *shiftpos,float *shiftdetect)const
{
  ...
  if(npbok){
    //-Interaction of type Bound-Fluid / Interaccion Bound-Fluid
    InteractionForcesBoundMP      <psimple,tker,ftmode> (npbok,0,nc,hdiv,cellfluid,begincell,cellzero,dcell,pos,pspos,velrhop,code,idp,viscdt,ar,phasedata);
  }
}
//£££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££//MP
    ...
    if(!WithFloating || CODE_GetType(code[p])==CODE_TYPE_FLUID){//-Fluid Particles / Particulas: Fluid
      ...
      velrhopnew[p]=velrhop1[p];
		for(int iphase=0; iphase<int(MkListSize); iphase++){
			if(CODE_GetTypeAndValue(code[p])==phasedata[iphase].MkValue) rhop01=phasedata[iphase].PhaseRhop0;
		}
...
//==============================================================================
/// Calculate new values of density and set velocity=zero for cases of  
/// (fixed+moving, no floating). Changed initial density to deal with multiple phases
//==============================================================================
...
void JSphCpu::ComputeSymplecticPreMP(double dt){
  if(TShifting)ComputeSymplecticPreMPT<true> (dt);
  else         ComputeSymplecticPreMPT<false>(dt);
}
//==============================================================================
template<bool shift> void JSphCpu::ComputeSymplecticPreMPT(double dt){
  TmcStart(Timers,TMC_SuComputeStep);
  //-Assign memory to variables Pre / Asigna memoria a variables Pre.
  PosPrec=ArraysCpu->ReserveDouble3();
  VelrhopPrec=ArraysCpu->ReserveFloat4();
  ...
  for(int p=0;p<npb;p++){
	for(int iphase=0; iphase<int(MkListSize); iphase++){
		if(CODE_GetTypeAndValue(Codec[p])==PhaseDatac[iphase].MkValue) rhop0=PhaseDatac[iphase].PhaseRhop0;
	}
    const tfloat4 vr=VelrhopPrec[p];
    const float rhopnew=float(double(vr.w)+dt05*Arc[p]);
...
void JSphCpu::ComputeSymplecticCorrMP(double dt){
  if(TShifting)ComputeSymplecticCorrMPT<true> (dt);
  else         ComputeSymplecticCorrMPT<false>(dt);
}
//==============================================================================
template<bool shift> void JSphCpu::ComputeSymplecticCorrMPT(double dt){
  TmcStart(Timers,TMC_SuComputeStep);
...
//==============================================================================
  ...
  ,const unsigned *beginendcell,tint3 cellzero,const unsigned *dcell
  ,const tdouble3 *pos,const tfloat3 *pspos,const tfloat4 *velrhop,const word *code,const unsigned *idp
  ,tfloat3 *ace,tsymatrix3f *surf,StPhaseData *phasedata,tfloat3 *shiftpos,float *shiftdetect,double dt)const{
  ...
  for(int p1=int(pinit);p1<pfin;p1++){
	...
	for(unsigned iphase=0; iphase<MkListSize; iphase++){ //added selection for material properties
		if(CODE_GetTypeAndValue(code[p1])==phasedata[iphase].MkValue){
			phasetype1=phasedata[iphase].PhaseType;
			massp1=phasedata[iphase].PhaseMass;
    ...
    const tfloat3 velp1=TFloat3(velrhop[p1].x,velrhop[p1].y,velrhop[p1].z);
    const float rhopp1=velrhop[p1].w;
    tfloat3 acep1=TFloat3(ace[p1].x,ace[p1].y,ace[p1].z);
    tfloat3 shiftposp1=TFloat3(0);
    float pdiv=0;
      ...
      for(int y=yini;y<yfin;y++){
        ...
        for(unsigned p2=pini;p2<pfin;p2++){
          ...
          if(rr2<=Fourh2 && rr2>=ALMOSTZERO){
			...
			//-Tensile Instability - Monaghan
			float tens;
            if(tker==KERNEL_Wendland)GetTensWendland(wab,tens);
            else if(tker==KERNEL_Cubic)GetTensCubic(wab,tens);

			//-Concentration gradient
			float consub=shiftdetect[p2]-shiftdetectp1;
			shiftposp1.x+=consub*vol2*frx*(1.f+tens);
			shiftposp1.y+=consub*vol2*fry*(1.f+tens);
			shiftposp1.z+=consub*vol2*frz*(1.f+tens);

			//-Particle Divergence
			if(phasetype1==phasetype2) pdiv+=vol2*(drx*frx+dry*fry+drz*frz);

			//===== Surface Tension =====
			if (surfcoef1){
				tsymatrix3f temp;
				temp.xx=vol2*(surf1.xx+surf2.xx)/rhopp1;//(pi_surf1.xx/(rhopp1*rhopp1)+pi_surf2.xx/(velrhop2.w*velrhop2.w)); 
				temp.xy=vol2*(surf1.xy+surf2.xy)/rhopp1;//(pi_surf1.xy/(rhopp1*rhopp1)+pi_surf2.xy/(velrhop2.w*velrhop2.w));
				temp.xz=vol2*(surf1.xz+surf2.xz)/rhopp1;//(pi_surf1.xz/(rhopp1*rhopp1)+pi_surf2.xz/(velrhop2.w*velrhop2.w));
				temp.yy=vol2*(surf1.yy+surf2.yy)/rhopp1;//(pi_surf1.yy/(rhopp1*rhopp1)+pi_surf2.yy/(velrhop2.w*velrhop2.w));
				temp.yz=vol2*(surf1.yz+surf2.yz)/rhopp1;//(pi_surf1.yz/(rhopp1*rhopp1)+pi_surf2.yz/(velrhop2.w*velrhop2.w));
				temp.zz=vol2*(surf1.zz+surf2.zz)/rhopp1;//(pi_surf1.zz/(rhopp1*rhopp1)+pi_surf2.zz/(velrhop2.w*velrhop2.w));
	...
	if(phasetype1!=1 && abs(pdiv)<ShiftTFS)ShiftingFreeSurface(shiftposp1,congr1);
	else {congr1.x=shiftposp1.x; congr1.y=shiftposp1.y; congr1.z=shiftposp1.z;}
    //-Sum results together / Almacena resultados.
	congr1.x*=diffc; congr1.y*=diffc; congr1.z*=diffc;
    shiftpos[p1]=shiftpos[p1]+congr1;
	ace[p1]=acep1;
  }
}
//£££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££££//MP
...
void JSphCpu::RunShiftingMP(unsigned np,unsigned npb,unsigned npbok
  ...
  if(TKernel==KERNEL_Wendland){          const TpKernel tker=KERNEL_Wendland;
    RunShiftingInteractionMP<psimple,tker> (npf,npb,nc,hdiv,cellfluid,begincell,cellzero,dcell,pos,pspos,velrhop,code,idp,ace,surf,phasedata,shiftpos,shiftdetect,dt);
  }
  else  if(TKernel==KERNEL_Cubic){          const TpKernel tker=KERNEL_Cubic;
    RunShiftingInteractionMP<psimple,tker> (npf,npb,nc,hdiv,cellfluid,begincell,cellzero,dcell,pos,pspos,velrhop,code,idp,ace,surf,phasedata,shiftpos,shiftdetect,dt);
  }
