...
__device__ void KerGetKernelShift(float rr2,float drx,float dry,float drz
  ,float &frx,float &fry,float &frz,float &wab,const double Dp1, const double Dp2, const double H1, const double H2, float Awen1, float Awen2, float Bwen1, float Bwen2 )
{
  const float rad=sqrt(rr2);
  ...
  const float fac1=Bwen2*qq2*wqq2*wqq2*wqq2/rad;
  const float fac2=Bwen1*qq1*wqq1*wqq1*wqq1/rad;
  ...
  ,TpShifting tshifting,float &shiftdetectp1)
{
  for(int p2=pini;p2<pfin;p2++){
    ...
    if(rr2<=CTE.fourh2 && rr2>=ALMOSTZERO){
      ...
      //if(tker==KERNEL_Wendland)KerGetKernelShift(rr2,drx,dry,drz,frx,fry,frz,wab,Dp1,Dp2,H1,H2); //ABVR
...
template<bool psimple,TpKernel tker,TpFtMode ftmode,bool lamsps,TpDeltaSph tdelta,bool shift> __device__ void KerInteractionForcesFluidBoxMP
  ...
  ,TpShifting tshifting,float3 &shiftposp1,float &shiftdetectp1,float &Sum,double Dp1,double H1, float Awen1, float Bwen1, float Fourh2) //ABVR
{
  for(int p2=pini;p2<pfin;p2++){
    float drx,dry,drz,pressp2;
    KerGetParticlesDr<psimple> (p2,posxy,posz,pospress,posdp1,posp1,drx,dry,drz,pressp2);
    ...
    if(rr2<=Fourh2 && rr2>=ALMOSTZERO){
	  ...
	  float rhop02; float cs02; float massp2; unsigned phasetype2; float gamma2; float visco2; double Dp2; double H2; float Awen2; float Bwen2;  
	  for(unsigned iphase=0; iphase<CTE.phasecount; iphase++){ 
		  if(CODE_GetTypeAndValue(code[p2])==accdata[iphase].MkValue){
			  ...
			  Bwen2=accdata[iphase].PhaseAwen; //ABVR
			  break;
		  }
	  }
      //-Wendland or Cubic Spline kernel.
      float frx,fry,frz,wab;
      if(tker==KERNEL_Wendland)KerGetKernelShift(rr2,drx,dry,drz,frx,fry,frz,wab,Dp1,Dp2,H1,H2,Awen1,Awen2,Bwen1,Bwen2); //ABVR
      else if(tker==KERNEL_Cubic)KerGetKernelCubicShift(rr2,drx,dry,drz,frx,fry,frz,wab);

	  // ABVR
	  if (idp[p1] == 386) Sum+=H1;
	  //Sum+=(massp2/rhop02)*wab;
	  if (idp[p1] == 775) Sum+=H2;
      //-Obtiene masa de particula p2 en caso de existir floatings.
      //-Obtains mass of particle p2 if any floating bodies exist.
      bool ftp2;         //-Indica si es floating. //-indicates if it is floating.
      float ftmassp2;    //-Contiene masa de particula floating o massp2 si es bound o fluid. //-Contains mass of floating body or massf if fluid.
  ...
  ,TpShifting tshifting,float3 *shiftpos,float *shiftdetect)
{
  ...
  if(p<n){
	...
	//-Load unique properties for each phase
	float rhop01; float cs01; unsigned phasetype1; float gamma1; float visco1; float massp1; float surfc1=0.f; double Dp1; double H1; float Awen1; float Bwen1; float Fourh2; //ABVR
	for(unsigned iphase=0; iphase<CTE.phasecount; iphase++){
    ...
    if(shift){
      shiftposp1=make_float3(0,0,0);
	  shiftdetectp1=(tker==KERNEL_Wendland? massp1*Awen1/rhopp1:massp1*CTE.cubic_a2/rhopp1); //ABVR
    }
    ...
    //-Obtains interaction limits
		...
		KerGetInteractionCellsMPVR(dcell[p1],hdivnew,nc,cellzero,cxini,cxfin,yini,yfin,zini,zfin);
